...
export function AuthDialog({ open, mode, onClose }) {
  ...
  return (
    <div
      ...
      aria-label={mode === 'login' ? 'Sign in' : 'Open account'}
      style={{
    ...
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      {step === 1 && (
        <form onSubmit={next} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <Input
            label="Full name" required value={data.name}
            onChange={(e) => setData({ ...data, name: e.target.value })}
            iconLeft={<Icon.user size={14} />} placeholder="Maya Chen"
          />
          <Input
            label="Email" type="email" required value={data.email}
            onChange={(e) => setData({ ...data, email: e.target.value })}
            iconLeft={<Icon.mail size={14} />} placeholder="you@example.com"
          />
          <Button type="submit" size="lg" loading={loading} style={{ width: '100%', marginTop: 4 }} iconRight={<Icon.arrow size={14} />}>
            Continue
          </Button>
      ...
      {step === 2 && (
        <form onSubmit={next} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Button type="submit" size="lg" loading={loading} style={{ width: '100%' }} iconRight={<Icon.arrow size={14} />}>
            Verify
          </Button>
      ...
      {step === 3 && (
        <form onSubmit={next} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {[
            ...
            <button
              key={f.v} type="button"
              onClick={() => setData({ ...data, funding: f.v })}
          ...
          <Button
            type="submit" size="lg" loading={loading} disabled={!data.funding}
            style={{ width: '100%' }} iconRight={<Icon.arrow size={14} />}
          >
            Continue
          </Button>
        </form>
