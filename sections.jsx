...
/* ============================================================
   ...
   ============================================================ */
function NavBar({ onCmdK, onAuthOpen }) {
  ...
  return (
    <header style={{
      ...
      background: scrolled ? 'oklch(0.16 0.012 250 / 0.7)' : 'transparent',
      ...
      <div className="container" style={{
        ...
        <nav aria-label="Primary" style={{ display: 'none', gap: 4 }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          >
            <Icon.search size={13} />
            <span style={{ display: 'none' }} className="cmdk-text">Search…</span>
            <KeyHint keys={['⌘', 'K']} />
          </button>
          ...
          <button
            aria-label="Menu" className="nav-mobile-toggle"
            onClick={() => setMobile((v) => !v)}
            style={{
              display: 'inline-flex', width: 36, height: 36, borderRadius: 8,
              background: 'var(--surface)', border: '1px solid var(--line)',
              alignItems: 'center', justifyContent: 'center', color: 'var(--text-2)',
            }}
          >{mobile ? <Icon.x /> : <Icon.menu />}</button>
        </div>
      </div>
      {mobile && (
        <div style={{ borderTop: '1px solid var(--line)', padding: '8px 18px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          {links.map((l) => (
            <a key={l.label} href={l.href} onClick={() => setMobile(false)}
              style={{ padding: '10px 12px', fontSize: 14, color: 'var(--text-2)', borderRadius: 8 }}>{l.label}</a>
          ))}
        </div>
      )}
      <style>{`
        @media (min-width: 880px) {
          ...
          .cmdk-text { display: inline !important; }
        }
      `}</style>
    </header>
  );
...
/* ============================================================
   Hero
   ============================================================ */
function Hero({ onAuthOpen, layout }) {
  const useSplit = layout === 'split';

  // generate believable portfolio chart data
  ...
  return (
    <section style={{ position: 'relative', overflow: 'hidden', paddingTop: 56 }}>
      ...
      <div className="container" style={{
        ...
        <div className="rise" style={{ textAlign: useSplit ? 'left' : 'center', maxWidth: useSplit ? 'none' : 880, marginInline: useSplit ? '0' : 'auto' }}>
...
function HeroChart({ chartData }) {
  ...
  return (
    <Card padded={false} elevation={3} className="rise" style={{ overflow: 'hidden', position: 'relative' }}>
      <div style={{
...
/* ============================================================
   Trust strip — partner wordmarks (original, monochrome)
   ============================================================ */
function TrustStrip() {
  const partners = [
    'NORTHGATE BANK', 'CIRRUS CAPITAL', 'MERIDIAN ALLOY',
    'PYTHIA RESEARCH', 'WELLBROOK', 'ATLAS PRIME', 'OMEGA DESK',
  ];
  return (
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 16 }}>
      <div>
        <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          width: 36, height: 36, borderRadius: 9, background: 'var(--accent-soft)', color: 'var(--accent)',
          marginBottom: 14,
        }}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
      ...
      <div className="mono" style={{
        ...
        background: 'var(--bg-2)', border: '1px solid var(--line)', color: 'var(--text-2)', marginTop: 12,
      }}>
...
/* ============================================================
   Portfolio simulator — slider-driven, with disclosure
   ============================================================ */
function Simulator() {
  const [deposit, setDeposit] = React.useState(25000);
  const [horizon, setHorizon] = React.useState(36); // months
  const [risk, setRisk] = React.useState('balanced');

  // simulated annualized return RANGES (low, mid, high) — historical-style, honest
  const profile = {
    ...
    <section style={{ paddingBlock: 'var(--section-pad)', background: 'var(--bg-2)', borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)' }}>
      <div className="container">
        <SectionHeading
          ...
          subtitle="Project a deposit across our risk-tiered portfolios using historical return ranges. This is illustrative only — past performance does not guarantee future results."
        />
...
/* ============================================================
   Pricing — Standard / Active / Elite (account tiers)
   ============================================================ */
function Pricing({ onAuthOpen }) {
  const tiers = [
    {
      name: 'Standard',
      ...
      sub: 'No monthly fee',
      features: [
        '$0 commissions on US equities',
        '0.20% / 0.30% spot crypto (maker / taker)',
        'Real-time data on 8 exchanges',
        'Mobile + web apps',
    ...
    },
    {
      ...
      features: [
        ...
        '0.10% / 0.20% spot crypto',
        'Advanced charting + 120 indicators',
        'API access (REST + WebSocket)',
        'Priority routing + lower spreads',
        'Live chat, 30-minute SLA',
      ],
      cta: 'Start 30-day trial',
    },
    {
      name: 'Elite',
      tag: 'For institutions & pros',
      price: 'Custom',
      sub: '50+ contact volume',
      features: [
        'Everything in Active',
        'Dedicated trading desk',
        'OTC liquidity & block routing',
        'FIX 4.4 connectivity',
        'White-glove onboarding',
        'Custom reporting + audit exports',
      ],
      cta: 'Talk to sales',
    },
  ];
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }} className="pricing-grid">
          {tiers.map((t) => (
            <Card key={t.name} padded={false} elevation={t.popular ? 3 : 1}
              style={{
                position: 'relative',
                borderColor: t.popular ? 'var(--accent-line)' : 'var(--line)',
                ...(t.popular ? { boxShadow: 'var(--shadow-pop), var(--glow)' } : {}),
              }}
            >
              {t.popular && (
                <div style={{
...
/* ============================================================
   Dashboard preview — interactive, 3 tabs
   ============================================================ */
function DashboardPreview() {
  const [tab, setTab] = React.useState('overview');
  return (
    <section id="markets" style={{ paddingBlock: 'var(--section-pad)' }}>
      <div className="container">
        <SectionHeading
...
function DashOverview() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 0 }} className="dash-grid">
      <div style={{ padding: 20, borderRight: '1px solid var(--line)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 18 }}>
          <div>
            ...
            <div className="mono" style={{ fontSize: 30, fontWeight: 600, letterSpacing: '-0.02em', marginTop: 4 }}>
              $147,408.22
            </div>
          </div>
      ...
      <style>{`@media (max-width: 880px) { .dash-grid { grid-template-columns: 1fr !important; } .dash-grid > div { border-right: 0 !important; border-bottom: 1px solid var(--line); } }`}</style>
    </div>
  );
}
          ...
          {rows.map((r) => (
            ...
            >
              <td style={{ padding: '12px 16px' }}>
                <span className="mono" style={{ fontSize: 12, fontWeight: 600 }}>{r.sym}</span>
              </td>
              ...
              <td className="mono" style={{
                padding: '12px 16px', textAlign: 'right', fontSize: 13,
                color: r.ch >= 0 ? 'var(--pos)' : 'var(--neg)',
              }}>
...
function DashPortfolio() {
  ...
  const segs = allocs.map((a) => {
    const start = acc; acc += a.value;
    return { ...a, start, end: acc };
  });
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }} className="dash-grid">
        ...
        {[
          ...
          <div key={i} style={{
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
              <Badge tone={a.type === 'BUY' || a.type === 'DEPOSIT' || a.type === 'DIV' ? 'positive' : 'negative'} mono>
                {a.type}
              </Badge>
              <span className="mono" style={{ color: 'var(--text)' }}>{a.sym}</span>
            </span>
            <span className="mono" style={{ color: 'var(--text-3)', fontSize: 12 }}>{a.q} @ ${a.p.toLocaleString(undefined,{minimumFractionDigits:2})} · {a.t}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="container">
        <Card padded={false} elevation={3} style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            }}>Open an account in 90 seconds.</h2>
...
function Footer() {
  ...
  return (
    <footer style={{ borderTop: '1px solid var(--line)', paddingBlock: '64px 36px' }}>
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr repeat(4, 1fr)', gap: 32 }} className="footer-grid">
          <div>
            <Logo />
          ...
          {Object.entries(cols).map(([title, links]) => (
            <div key={title}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 9 }}>
                {links.map((l) => (
                  <li key={l}><a href="#" style={{ fontSize: 13.5, color: 'var(--text-3)', transition: 'color 180ms' }}
                    onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--text)')}
      ...
      <style>{`@media (max-width: 880px) { .footer-grid { grid-template-columns: 1fr 1fr !important; } }`}</style>
    </footer>
  );
}

Object.assign(window, { NavBar, Hero, TrustStrip, Features, Simulator, Pricing, Compare, DashboardPreview, CTA, Footer });
